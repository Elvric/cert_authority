...
if [ ! -f /etc/ca/intermediate/index.txt ]; then
...
openssl req -config /etc/ca/intermediate/intermediate.cnf -key /etc/ca/intermediate/certificates/client.key -new -sha256 -out /etc/ca/intermediate/certificates/client.csr -subj "/C=CH/ST=VD/L=Lausanne/O=IMovies/OU=CA/CN=$1_$2/emailAddress=$3"
