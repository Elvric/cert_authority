...
\begin{center}
{\large\textcolor{red}{Page limit: 25 pages.}}
\end{center}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\item Are pairwise distinct: certificate, private key, public key, archive of certificate with private key. Please avoid mixing these up.
\end{itemize}

\item Refer to the source document of your risk definitions if appropriate.

\item For the risk evaluation:
\begin{itemize}
\item Formulate the threats in active, not passive, voice: who (threat source) does what (threat action)? 
\item Avoid redundancies (e.g., rather than listing the same threats for every physical machine, collect 
them into a group and list the threats only once).
\item Number the threats consecutively.
\end{itemize}
...
the more technical description that follows. It must include a figure showing 
a network diagram with all relevant machines and services. 
...
List all potential threats and the corresponding countermeasures. Number them consecutively.
Estimate the risk based on the information about the threat, the threat sources and the corresponding countermeasure. Adhere to the risk definitions you have given above. As a sanity check, there should be at least one high-risk entry.
